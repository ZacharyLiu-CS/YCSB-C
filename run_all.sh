  ...
  for db in ${db_list[*]};do
    for workload in ${workload_list[*]};do
      for thread_count in ${thread_count_list[*]};do
        # echo the command 
        echo "numactl --cpunodebind=0 --membind=0 ./ycsbc -db ${db} -dbpath /mnt/pmem0/ycsb-${db} -P ../workloads/${workload}_${value_size}.spec -threads ${thread_count} > ${db}_thread_${thread_count}_value_${value_size}_${workload}.log 2>&1"
        # run the command
        numactl --cpunodebind=0 --membind=0 ./ycsbc -db ${db} -dbpath /mnt/pmem0/ycsb-${db} -P ../workloads/${workload}_${value_size}.spec -threads ${thread_count} > ${db}_thread_${thread_count}_value_${value_size}_${workload}.log 2>&1
        # remove the files
